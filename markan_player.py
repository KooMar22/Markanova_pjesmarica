...
class MediaPlayer:
    def __init__(self):
        ...
        self.playlist_frame = Frame(self.root, bg="light yellow")
        self.playlist_frame.grid(column=0, row=0, padx=10, pady=(0, 10))

        self.music_info_frame = Frame(self.root, bg="light yellow")
        self.music_info_frame.grid(column=0, row=1)

        self.control_panel = Frame(self.root, bg="light yellow")
        ...
        self.playlist_listbox = Listbox(self.playlist_frame, width=110, height=20,
                                selectbackground="gold", selectforeground="black",
                                selectmode=SINGLE, bg="light yellow")
        ...
        self.status_lbl = Label(self.music_info_frame, bg="light yellow",
                                font=("Arial", 12, "bold", "italic"),
                                textvariable=self.status_variable)
        self.status_lbl.grid(column=0, row=0, padx=10, pady=10, sticky="ew")

        # Add Progress Slider
        self.progress_slider = ttk.Scale(self.music_info_frame, length=620,
        ...
        self.song_time_lbl = Label(self.music_info_frame, text="", bg="light yellow")
        ...
        self.play_pause_btn = Button(self.control_panel, image=self.play_img,
                                     command=self.play_or_pause, bg="light yellow")
        self.play_pause_btn.image = self.play_img
        self.play_pause_btn.grid(column=2, row=0, padx=10, pady=10)

        # Stop button
        self.stop_btn = Button(self.control_panel, image=self.stop_img,
                               command=self.stop_song, bg="light yellow")
        self.stop_btn.image = self.stop_img
        self.stop_btn.grid(column=3, row=0, padx=10, pady=10)

        # Shuffle button
        self.shuffle_btn = Button(self.control_panel, image=self.shuffle_on_img,
                                  command=self.shuffle, bg="light yellow")
        self.shuffle_btn.image = self.shuffle_btn
        self.shuffle_btn.grid(column=0, row=0, padx=10, pady=10)

        # Backward button
        self.back_btn = Button(self.control_panel, image=self.back_img,
                               command=self.backward, bg="light yellow")
        self.back_img.image = self.back_img
        self.back_btn.grid(column=1, row=0, padx=10, pady=10)

        # Forward
        self.fwd_btn = Button(self.control_panel, image=self.fwd_img,
                              command=self.forward, bg="light yellow")
    ...
    def extract_song_info(self, file_path):
        """Extracts song metadata (artist, title and song length)"""
        try:
            audio = MP3(file_path)
            ...
            self.total_time = "{:02d}:{:02d}".format(mins, secs)
            return {"artist": artist, "title": title, "total_time": self.total_time}
    ...
    def add_music(self):
        ...
        for file_path in files:
            ...
            if song_info:
                ...
                self.playlist_listbox.insert(END, f"{song_info['artist']} - {song_info['title']} - {song_info["total_time"]}")
    ...
    def update_time(self):
        ...
        if music_playlist:
            ...
            if int(self.progress_slider.get()) == int(self.song_len):
                ...
                self.song_time_lbl.config(text=f"{elapsed_time}")
